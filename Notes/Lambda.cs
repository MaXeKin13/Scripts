...
namespace NoteScripts
{
...

    {
        ...
        static void Main(string[] args)
        {
            ...
            DisplayPeople("Age > 20 with search keyword:" + searchKeyword, people, x => //x will recieve each 'Person' from the people list, because DisplayPeople iterates through it
            {
                
                if(x.Name.Contains(searchKeyword) && x.Age > 20)
                    return true;
            }
            else
            {
        ...
        static void DisplayPeople(string title, List<Person> people, FilterDelegate filter)
        {
            ...
            {
                if (filter(p)) // run the 'filter' code to return a bool
                {

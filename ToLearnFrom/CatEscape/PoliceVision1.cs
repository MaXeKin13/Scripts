...
public class PoliceVision : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        transform.position = owner.EyeTr.transform.position;
        ...
        if (timer > refresh_rate)
        {
            ...
            for (float i = -minmax; i <= minmax; i += step_jump)
            {
                ...
                bool ishit = Physics.Raycast(new Ray(pos_world, dir_world), out hit, vision_range, visionMask.value);
    ...
    {
        GetComponent<MeshRenderer>().material = seenMaterial;
    }
}
